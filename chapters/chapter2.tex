...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Cảnh báo khi độ tương đồng thấp hơn ngưỡng cho phép (< 98\%)
    ...
    \begin{itemize}
        ...
        \item Xác minh biển số xe khớp với thông tin đã lưu
        \item Cảnh báo khi độ tương đồng thấp hơn ngưỡng cho phép (< 85\%)
        \item Yêu cầu xác nhận từ chủ xe qua mobile app nếu phát hiện bất thường
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Siamese matching: $\geq 95\%$ (threshold similarity > 0.85)
    \end{itemize}
...
\begin{itemize}[leftmargin=*,nosep]
\item Nếu không tìm thấy xe: Hiển thị lỗi
\item Nếu đặc trưng xe không khớp (similarity < 0.85): Tạo cảnh báo , chờ xác nhận từ chủ xe
\item Nếu logo không khớp: Tạo cảnh báo 
\end{itemize}
\end{minipage} \\ \hline
\textbf{Hậu điều kiện} & Phiên đỗ xe kết thúc, barrier mở, thông tin được cập nhật \\ \hline
\end{tabular}
...
\item So sánh với ngưỡng (threshold = 0.85)
\item Trả về kết quả khớp/không khớp
\end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng thay thế} & Nếu similarity < 0.85, trả về không khớp và tạo cảnh báo \\ \hline
...
Nếu chưa có xác nhận từ mobile app, hệ thống hiển thị thông báo "Chưa có xác nhận từ chủ xe", tạo cảnh báo và gửi thông báo đến chủ xe. Người dùng nhận thông báo báo gộp và xem ảnh khuôn mặt người lái. Dựa trên xác nhận của chủ xe "Người đó là tôi" hoặc hết thời gian timeout, hệ thống quyết định cho phép xe ra hoặc giữ barrier đóng.
