...
\textbf{User:} Đại diện cho người dùng hệ thống với các thuộc tính id (str), email (str), full name (str), role (str: customer/barber/admin). Có quan hệ 1-N với Barber, Booking, Rating, Appointment và ChatSession.

\textbf{Barber:} Thông tin barber shop với id (UUID), name (str), address (str), rank (Decimal), user id (UUID). Quan hệ N-1 với User, 1-N với Service, TimeSlot và Rating.

\textbf{Service:} Dịch vụ cung cấp với id (int), barber id (str), service name (str), price (int), duration min (int). Quan hệ N-1 với Barber và M-N với Booking.

\textbf{TimeSlot:} Khung giờ làm việc với id (int), barber id (str), slot date (date), start time (time), end time (time), is available (bool). Quan hệ N-1 với Barber và 1-1 với Booking.

\textbf{Booking:} Lịch hẹn với id (int), user id (str), time slot id (int), total price (int), status (str: pending/confirmed/rejected/completed). Quan hệ N-1 với User, 1-1 với TimeSlot, M-N với Service.

\textbf{Rating:} Đánh giá với id (int), barber id (str), user id (str), score (Decimal: 1-5). Quan hệ N-1 với cả User và Barber.

\textbf{Appointment:} Yêu cầu hợp tác với id (str), user id (str), name barber (str), status (str: pending/approved/rejected). Quan hệ N-1 với User.

\textbf{ChatSession:} Phiên chat với id (str), user id (str), title (str). Quan hệ N-1 với User.
...
\begin{itemize}
    \item register user(): Đăng ký tài khoản mới, validate email uniqueness
    \item login user(): Xác thực người dùng, tạo session
    \item update user(): Cập nhật thông tin profile (avatar, phone, password)
\end{itemize}

\textbf{BarberService:} Quản lý barber shops với các methods:
\begin{itemize}
    \item create barber(): Tạo barber shop mới, validate owner
    \item get barber by id(): Lấy thông tin chi tiết barber
    \item update barber(): Cập nhật thông tin, services, working hours
    \item get top barbers(): Lấy danh sách barbers theo rank và location
\end{itemize}

\textbf{TimeSlotService:} Quản lý khung giờ với các methods:
\begin{itemize}
    \item create time slot(): Tạo khung giờ mới cho barber
    \item get available time slots(): Lấy danh sách khung giờ trống theo ngày và barber
\end{itemize}

\textbf{BookingService:} Xử lý đặt lịch với các methods:
\begin{itemize}
    \item create booking(): Tạo booking mới, lock time slot, tính total price
    \item get bookings by user(): Lấy lịch sử bookings của user
    \item cancel booking(): Hủy booking, release time slot
\end{itemize}
Dependency: TimeSlotService (kiểm tra availability trước khi book)

\textbf{RatingService:} Quản lý đánh giá với các methods:
\begin{itemize}
    \item create rating(): Tạo rating mới, validate booking completed
    \item get ratings by barber(): Lấy danh sách ratings của barber
    \item update barber rank(): Tính lại rank trung bình, trigger sau mỗi rating
\end{itemize}
...
\begin{itemize}
    \item query(): Xử lý query từ user, semantic search documents, generate response
    \item create chat session(): Tạo session mới cho user
    \item generate embedding(): Convert text thành vector embedding (768-dim)
\end{itemize}
...
\begin{itemize}
    \item process image(): Pipeline chính - preprocess, detect, analyze
    \item detect acne in region(): Detect mụn trong vùng face sử dụng YOLOv8 + CNN
\end{itemize}
Technologies: MediaPipe Face Mesh, YOLOv8, MobileNetV2 CNN

\textbf{HairStyleGenerator:} Tạo kiểu tóc với các methods:
\begin{itemize}
    \item generate single style(): Generate hairstyle sử dụng Stable Diffusion
    \item preprocess image(): Face detection, segmentation, mask preparation
\end{itemize}
